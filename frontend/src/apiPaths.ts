export const API_LOGIN_PATH = "./api/v1/auth/login";
export const API_SUMMARY_PATH = "/api/v1/summary";
export const API_NET_WORTH_GRAPH_PATH = "/api/v1/calculations/networthHistory";
export const API_TRANSACTIONS_PATH = "/api/v1/txns";
export const API_TRANSACTIONS_JSON_QUERY_PATH = "/api/v1/transactions/json-query";
export const API_CURRENCIES_PATH = "/api/v1/currencies";
export const API_CONTAINERS_PATH = "/api/v1/accounts";
export const API_POST_CONTAINERS_PATH = "/api/v1/accounts";
export const API_CONTAINERS_TIMELINES_PATH = `/api/v1/accounts/timeline`;
export const API_TXN_TAGS_PATH = "/api/v1/txnTags";
export const API_GRAPHS_PATH = "/api/v1/graphs";
export const API_BAL_VAL_PATH = "/api/v1/balanceHistory";
export const API_DASHBOARD_BATCH_PATH = `/api/v1/batch/dashboard`;
export const API_USER_INCOMES_EXPENSES_PATH = `/api/v1/calculations/expensesAndIncomes`;
export const API_CURRENCY_RATE_HISTORY_PATH = `/api/v1/currencies/history`;
export const API_PUT_TRANSACTIONS_PATH = "/api/v1/txns";
export const API_DELETE_TRANSACTIONS_PATH = "/api/v1/txns";
export const API_GET_CURRENCIES_RATE_SRCS_PATH = "/api/v1/currencyRateSources";
export const API_PATCH_CURRENCIES_RATE_SRCS_PATH = "/api/v1/currencyRateSources";
export const API_POST_CURRENCIES_RATE_SRCS_PATH = "/api/v1/currencyRateSources";
export const API_FILES_INIT_SESSION_PATH = "/api/v1/files/init";
export const API_FILES_APPEND_CHUNK_PATH = "/api/v1/files/append";
export const API_FILES_DOWNLOAD_PATH = "/api/v1/files/view";
export const API_FILES_LIST_PATH = "/api/v1/files";
export const API_FILE_BY_ID_PATH = "/api/v1/files/id";