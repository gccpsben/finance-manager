...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        {
            table = table.col(ColumnDef::new(CurrencyRateDatum::Id)
                .uuid()
                .not_null()
                .primary_key());
        }
        ...
        {
            table = table.col(ColumnDef::new(CurrencyRateDatum::Amount)
                .string()
                .not_null());
        }

        {
            table = table.col(ColumnDef::new(CurrencyRateDatum::RefCurrencyId)
                .uuid()
                .not_null()
            );
        ...
        {
            table = table.col(ColumnDef::new(CurrencyRateDatum::RefAmountCurrencyId)
                .uuid()
                .not_null()
            );
            table = table.foreign_key(
                ...
                .take()
                .from(CurrencyRateDatum::Table, CurrencyRateDatum::RefAmountCurrencyId)
                .to(Currency::Table, Currency::Id)
            );
        }

        {
            table = table.col(ColumnDef::new(CurrencyRateDatum::OwnerId)
                .uuid()
                .not_null()
            );
        ...
        {
            table = table.col(ColumnDef::new(CurrencyRateDatum::Date)
                .integer()
                .not_null())
                .index(
                    Index::create()
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.drop_index(Index::drop().if_exists().name(OWNER_CURR_DATE_UNIQUE_INDEX_NAME).to_owned()).await?;
        manager
